...
```{r}
 get_CV_fat_rate_table(clean_fat_data,
                       save = F)%>% 
 kableExtra::kbl() %>% 
   kableExtra::kable_styling() %>% 
   kableExtra::scroll_box(width = "100%", height = "250px")
...
+ __season 1__: early_dry (season): 1-6, dry (season): 7-12 , or
...
Since it seems that season 1 allows for more balanced categories, and their is no difference for the number of carcasses per species per season1/season 2, I'll use a the 'season 1' way to divide the year afterwards
...
Since season 1 is more balanced than season 2, I use 'season1' (early_dry: 1-6, dry : 7-12)
...
+ Explanatory variable : season, qualitative
...
Number of emaciated carcasses/individuals lower during early dry season, increase during dry season.  
The observe the reverse for the thin and good carcasses/individual.  
--> Proportion of emaciated individual dry > early dry?  
...
```
More emaciated individuals/carcasses (<20) during dry season vs early dry.   
We observed the reverse for good condition carcasses/ indiv (>70) in early dry vs dry season.  
Thin individuals.carcasses : only a small change.  


2 variables qualitative --> chi square test ? 
